...
- 사용자 정보 수정 기능은 MVP 범위에서 제공하지 않는다
- (고정) 인증 정보(이메일)는 수정 대상이 아니다
...
- 사용자 상태 변경은 MVP 범위에서 API로 제공하지 않는다
...
- 모임 생성자는 참여 조건을 선택적으로 설정할 수 있다
- 참여 조건은 모임 조회 필터링에 사용된다
- 참여 조건은 다음 항목만을 사용한다:
    - 반려견 크기(meetups.dog_size)
    - 사회성(meetups.sociability_level)
    - 반응성(meetups.reactivity_level)
    - 중성화 여부(meetups.neutered)
...
참여 요청 적합성(Eligibility) (MVP)

- 모임에 참여 조건이 설정된 경우, 참여 요청자는 해당 조건을 만족해야 참여 요청을 생성할 수 있다
- 조건 비교는 **요청자의 대표 반려견(Primary dog) 프로필**을 기준으로 수행한다
    - 대표 반려견: 해당 사용자의 반려견 중 created_at이 가장 빠른 1마리
- 모임 참여 조건의 각 항목은 다음과 같이 동작한다:
    - 모임 조건 값이 null(미설정)이면 해당 항목은 비교에서 제외한다
    - 모임 조건 값이 설정되어 있으면, 요청자의 대표 반려견 값이 정확히 일치해야 한다
...
- 2026-02-10: User 정보 수정(1.3)은 MVP 제외로 확정. ERD(users)에는 email/status/created_at만 존재하며, MVP에서는 user profile 필드를 추가하지 않는다.
